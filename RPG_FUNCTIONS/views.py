...
# Попытка отыграть через TemplateView
class HomePageView(TemplateView):
    template_name = "otklick.html"

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["latest_articles"] = self.get_queryset_try()
        return context

    def get_queryset_try(self):
        ask_2 = list(GameUser.objects.filter(user_link=self.request.user).values_list('user_link', flat=True))[0]
        ...
        for x in list_1:
            a_1 = GamePost_message.objects.filter(text_message_senders__id=x).values_list('text_message_senders', flat=True)
            author_otklick += a_1
...
# #отдельная функция, тест
# def get_queryset_otklick(request):
#     ask_2= list(GameUser.objects.filter(user_link=request.user.id).values_list('user_link', flat=True))[0]
#
#     list_1= list(GamePost.objects.filter(link_GameUser = ask_2).values_list('id', flat=True))
#     author_otklick = []
#     text_otklick = []
...
#
#        # Возвращает отправителя сообщения:
#         a_1 = GamePost_message.objects.filter(text_message_senders__id=x).values_list('id', flat=True)
#         author_otklick += a_1
#        # Возвращается текст отправителя сообщения
#         t_1 = GamePost_message.objects.filter(text_message_senders__id=x).values_list('text_message', flat=True)
#         text_otklick += t_1
#     return HttpResponse(f"{author_otklick} and {text_otklick}")